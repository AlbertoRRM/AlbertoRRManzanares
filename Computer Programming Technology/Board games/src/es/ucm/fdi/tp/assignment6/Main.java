...
import es.ucm.fdi.tp.assignment5.attt.ATTTFactoryExt;
import es.ucm.fdi.tp.assignment5.connectN.ConnectNFactoryExt;
import es.ucm.fdi.tp.assignment5.ttt.TTTFactoryExt;
...
import es.ucm.fdi.tp.basecode.connectNsquare.ConnectNSquareFactory;
/**
 ...
 * informaciÃ³n sobre esta libreria en
 * {@link https://commons.apache.org/proper/commons-cli/} .
 */
public class Main {
	 ...
	 */
	enum GameInfo {
		CONNECTN("cn", "ConnectN"), ConnectNSquare("cns", "ConnectNSquare"), TicTacToe("ttt",
				"Tic-Tac-Toe"), AdvancedTicTacToe("attt", "Advanced Tic-Tac-Toe"), Ataxx("ataxx", "Ataxx");
	 ...
	 */
	final private static String DEFAULT_HOSTNAME = "localhost";
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructServerPortOption()); // -sp or
																// --server-port
		cmdLineOptions.addOption(constructHostNameOption()); // -sh or
																// --server-host
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case ConnectNSquare:
			if (dimRows != null && dimCols != null && dimRows == dimCols)
				gameFactory = new ConnectNSquareFactory(dimRows);
			else
				gameFactory = new ConnectNSquareFactory();
			break;
		case Ataxx:
			if (dimRows != null && dimCols != null && dimRows == dimCols) {
				if (obstacles != null)
	...
	private static Option constructObstacleOption() {
		return new Option("o", "obstacle", true, "The number of obstacles that will appear in each quadrant.");
	}

	/**
	 * Parses the obstacles option (-o or --obstacle). It sets the value of
	 * {@link #obstacles} accordingly.
	 * 
	 * <p>
	 * Extrae la opcion obstáculos (-o). Asigna el valor del atributo
	 * {@link #obstacles}.
	 * 
	 * @param line
	 *            CLI {@link CommandLine} object.
	...
	private static Option constructAppModeOption() {
		return new Option("am", "app-mode", true, "The mode of the application: normal, client or server");
	}
	 ...
	 * 
	 * <p>
	 * Extrae la opcion app mode (-am or --app-mode). Asigna el valor del atributo
	...
	private static void parseAppModeOption(CommandLine line) throws ParseException {
		String appMode = line.getOptionValue("am");

		if (appMode != null) {
			if (appMode.equals(AppMode.NORMAL.getId()))
				applicationMode = AppMode.NORMAL;
			else if (appMode.equals(AppMode.CLIENT.getId()))
				applicationMode = AppMode.CLIENT;
			else if (appMode.equals(AppMode.SERVER.getId()))
				applicationMode = AppMode.SERVER;
			else
				throw new ParseException("Invalid application mode option: " + appMode);
		} else
			applicationMode = DEFAULT_APPMODE;
	}

	private static Option constructServerPortOption() {
		return new Option("sp", "server-port", true, "The port of the server");
	}

	/**
	 * Parses the server port option (-sp or --server-port). It sets the value of
	 * {@link #serverPort} accordingly.
	 * 
	 * <p>
	 * Extrae la opcion server port (-sp or --server-port). Asigna el valor del atributo
	 * {@link #serverPort}.
	 * 
	 * @param line
	 *            CLI {@link CommandLine} object.
	...
	private static void parseServerPortOption(CommandLine line) throws ParseException {
		String serverPortN = line.getOptionValue("sp");

		if (serverPortN != null) {
			try {
				serverPort = Integer.parseInt(serverPortN);
			} catch (NumberFormatException e) {
				throw new ParseException("Invalid number for the port: " + serverPortN);
			}
		} else
			serverPort = DEFAULT_SERVERPORT;
	}

	private static Option constructHostNameOption() {
		return new Option("sh", "server-host", true, "The name of the host");
	}
	
	/**
	 * Parses the server host option (-sh or --server-host). It sets the value of
	 * {@link #serverHost} accordingly.
	 * 
	 * <p>
	 * Extrae la opcion server host (-sh or --server-host). Asigna el valor del atributo
	 * {@link #serverHost}.
	 * 
	 * @param line
	 *            CLI {@link CommandLine} object.
	 ...
	 *             Si se proporciona un valor invalido.
	 */
	private static void parseHostNameOption(CommandLine line) {
		String hostName = line.getOptionValue("sh");
	...
	public static void startGame() {
		...
		case WINDOW:
			...
			// If we are in multiple view mode, we create a window for each
			// player:
			if (multiviews == true) {
				for (Piece p : pieces) {
			...
			// If we are in single view mode, we only create a window:
			else
	...
	/**
	 * Starts a new client. It passes the server host name and server port to create it.
	 */
	public static void startClient() {
		try {
			...
			System.err.println("The client could not be started.");
		}
	}

	/**
	 * Starts a new server. It passes the gameFactory, the pieces and a server port to create it.
	 */
