...
bool createAccount(tManager &manager)
{
	...
	initialize(manager.users.list[manager.activeUser].mail_inbox);
	initialize(manager.users.list[manager.activeUser].mail_outbox);
...
void sendMail(tManager &manager, const tMail &mail)
{
	...
	if (manager.users.list[manager.activeUser].mail_inbox.counter < Max_Ref)  //If there is enough space in the list of references and the user exists...
	{
...
void hidden_password(string &password)
{
	string password[10];
}
