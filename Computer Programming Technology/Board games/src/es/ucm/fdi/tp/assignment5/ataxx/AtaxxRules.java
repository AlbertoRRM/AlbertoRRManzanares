 ...
 * cell at distance 1 or 2. If the distance is 1, the piece is replicated, if it is 2, it is only moved. 
 * The winner is the one who achieves the greater amount of pieces when the board is completely filled.</li>
 * </ul>
 */
public class AtaxxRules implements GameRules {
	...
	/**
	 * Number of obstacles in the game.
	 */
	...
	public Board createBoard(List<Piece> pieces) {
		...
		// This locates the initial pieces in default positions.
		...
		// If the number of obstacles is greater than the 10% of positions, force it to be 10%.
		...
		for (int i = 0; i < this.obstacles; i++) {
			...
			}// The second and third conditions check if the row/column is not the middle one, because I want to locate the obstacles
			// in the quadrants
			while ((board.getPosition(row, col) != null || (row == (int)(board.getRows()/2))) || (col == (int)(board.getCols()/2)));
	 ...
	 * 
	 * @param l is the list of pieces involved in the game
	...
	public Pair<State, Piece> updateState(Board board, List<Piece> playersPieces, Piece lastPlayer) {
		if (nextPlayer(board, playersPieces, lastPlayer) == null) {
			...
			for (int i = 0; i < playersPieces.size(); i++) {
				...
				// If the pieces is the maximum amount, the winner could be the current player
				if (count > max)  {
					winner = p;
					max = count;
				} else if (count == max) {
					tie = count;// If two player have the maximum amount of pieces and there are
	...
	public Piece nextPlayer(Board board, List<Piece> playersPieces, Piece lastPlayer) {
		...
		while(nextPlayer != lastPlayer) {
			if(validMoves(board, playersPieces, nextPlayer).size() != 0) // If the list is empty look for another player
				return nextPlayer;
			else 
				i++;
