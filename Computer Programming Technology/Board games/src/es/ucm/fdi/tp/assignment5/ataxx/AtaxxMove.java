...
public class AtaxxMove extends GameMove {
	...
	/**
	 * Constructor with parameters for AtaxxMove
	 * 
	 * @param init_row
	 *            is the initial row.
	 * @param init_col
	...
	public void execute(Board board, List<Piece> pieces) {
		...
		if (getPiece().equals(board.getPosition(init_row, init_col))) { // If
																		// there
																		// is a
																		// player's
																		// piece
			if (board.getPosition(final_row, final_col) == null) { // If the
																	// destination
																	// coordinates
																	// are not
																	// occupied
				if ((row_distance <= 2) && (column_distance <= 2)) // If the
																	// distance
																	// is lower
																	// or equal
																	// than 2
				{
					board.setPosition(final_row, final_col, getPiece()); // Set
																			// the
																			// player's
																			// piece
		...
		for (int i = -1; i <= 1; i++) {
			for (int j = -1; j <= 1; j++) {
				...
				if (final_row + i >= 0 && final_col + j >= 0 && final_row + i < board.getRows()
					...
					if ((p != null) && !(getPiece().equals(p))) {
						for (int k = 0; k < pieces.size(); k++) {
							if (p.getId() == pieces.get(k).getId())
